...
async def find_device():
    ...
    # Set velocity limit for torque mode (safety feature) - 15 mph max
    max_speed_rad_s = mph_to_rad_per_sec(15.0)
    await odrv0.axis0.controller.config.vel_limit.write(max_speed_rad_s)  # rad/s
    print(f"✓ Velocity limit set to 15.0 mph ({max_speed_rad_s:.1f} rad/s)")
...
async def main():
    ...
    max_torque = 0.5  # Maximum torque in Nm - start conservative!
    torque_ramp_rate = 2.0  # Nm/s - how fast torque can change
    current_torque = 0.0
    dt = 0.05  # 20Hz update rate
    
    # Safety parameters - all set to 15 mph
    max_safe_mph = 15.0
    ...
    try:
        while True:
            ...
            if abs(axis_value) > 0.1:  # Only print when actively controlling
                print(f"Torque: {current_torque:.3f} Nm, Speed: {current_mph:.2f} mph ({current_velocity_rad_s:.2f} rad/s)")
